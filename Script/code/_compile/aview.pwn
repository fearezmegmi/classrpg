...
#define DIALOG_ID_ONE 	(455)
#define DIALOG_ID_TWO 	(456)
#define DIALOG_ID_THREE	(457)
#define DIALOG_ID_FOUR 	(458)
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
	...
	{
		ShowPlayerDialog(EditID, DIALOG_ID_ONE, DIALOG_STYLE_INPUT, "Save Animation", " {c4c4c4}Memorable comment... (optional)", "Next", "Close");
	}
	else
	{
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		case DIALOG_ID_ONE: if(response)
		{
			new string[120];
			GetAnimationName(save_Name, animlib, 32, animname, 32);			
		...
		case DIALOG_ID_TWO: if(response)
		{
			switch(listitem)
			{
				case 0: ShowPlayerDialog(EditID, DIALOG_ID_THREE, DIALOG_STYLE_INPUT, "Speed", "{c4c4c4}Specify speed. Default: 4.1", "Next", "Back");
				...
				case 4: ShowPlayerDialog(EditID, DIALOG_ID_FOUR, DIALOG_STYLE_INPUT, "Time", "{c4c4c4}Specify time. Default: 0", "Next", "Back");
				case 5: a_Forcesync = a_Forcesync ? false : true, ShowSettingDialog();
			}
			ClearAnimations(EditID, a_Forcesync);
			return 1;
		}
		case DIALOG_ID_THREE:
		{
			if(response)
			{
				new Float:speed = floatstr(inputtext);
				if(speed < 0.1000 || speed > 4.10000) return ShowPlayerDialog(EditID, DIALOG_ID_THREE, DIALOG_STYLE_INPUT, "Speed", "{ff0000}Error value!     {c4c4c4}0.1 — 4.1\nSpecify speed. Default: 4.1", "Next", "Back");
				a_Speed = speed + 0.001;				
			}
			ShowSettingDialog();
			return 1;
		...
		case DIALOG_ID_FOUR:
		{
			if(response)
			{
				...
				if(time < 0 || time > 255) return ShowPlayerDialog(EditID, DIALOG_ID_FOUR, DIALOG_STYLE_INPUT, "Time", "{ff0000}Error value!\n{c4c4c4}Specify time. Default: 0", "Next", "Back");
				a_Time = time;				
			}
			ShowSettingDialog();
			return 1;
...
stock ShowSettingDialog()
{
	...
	ShowPlayerDialog(EditID, DIALOG_ID_TWO, DIALOG_STYLE_LIST, "Setting", string, "Select", "Close");
}